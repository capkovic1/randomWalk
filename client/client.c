...
void client_run(void) {
  ...
  while (1) {
    ...
    case UI_SETUP_SIM: {
      timeout(-1);  
      UIState next = draw_setup(&x, &y, &K, &runs,&width, &height,probs, mode,out_filename, sizeof(out_filename));
      ...
      int success = send_config_to_server(&ctx,x, y,width, height,K, runs,probs,out_filename,&next);
